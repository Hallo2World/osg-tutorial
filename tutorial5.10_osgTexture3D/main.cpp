...
osg::ref_ptr<osg::StateSet> createState()
{
	...
	if (image_0->getPixelFormat() != image_1->getPixelFormat() ||
		image_1->getPixelFormat() != image_2->getPixelFormat() ||
		image_2->getPixelFormat() != image_3->getPixelFormat()) {
		std::cout << "Warning:image pixel formats not compatible." << std::endl;
		return new osg::StateSet();
	}

	// 得到支持的，最大的三维纹理单元的大小
	GLint textureSize = 256; // osg::Texture3D::getExtentions(0, true)->maxTexture3DSize();
	// 对4张二维纹理图像进行缩放，以达到相同的大小
	...
	// 第一个和第二个参数是纹理的大小，第三个参数是三维纹理数据图像的深度
