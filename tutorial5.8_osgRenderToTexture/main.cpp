	...
	bool useImage)
{
	...
	// 创建一个用来浏览的四边形几何体
	{
		...
		// 现在需要将纹理附加到几何体上，创建一个包含纹理的StateSet
	...
	// 需要创建一个相机节点，用来渲染到该纹理（RTT）
	{
		...
		// 设置相机渲染，通过 OpenGL frame buffer object 实现
...
int main()
{
	...
	// 设置更新回调
	osg::ref_ptr<osg::NodeCallback> nc = new osg::AnimationPathCallback(loadedModelTransform->getBound().center(), osg::Vec3(0.0f, 0.0f, 1.0f), osg::inDegrees(45.f));
	loadedModelTransform->setUpdateCallback(nc);

	osg::ref_ptr<osg::Group> root = new osg::Group();
	root->addChild(createPreRenderSubGraph(loadedModelTransform.get(), tex_width, tex_height, renderImplementation, useImage));
