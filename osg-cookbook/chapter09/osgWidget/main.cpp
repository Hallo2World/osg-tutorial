...
osgWidget::Label* createLabel(const string &name, const string &text, unsigned int size, const osg::Vec4& color)
{
...
osgWidget::Window* createSimpleTabs(float winX, float winY)
{
    ...
    for (unsigned int i = 0; i < 3; ++i) {
        osg::Vec4 color(0.0f, i / 3.0f, 0.0f, 1.0f);
        ...
        tab->addCallback(new osgWidget::Callback(&tabPressed, osgWidget::EVENT_MOUSE_PUSH, content));
        tabs->addWidget(tab);
    }
...
int main()
{
    ...
    osg::ref_ptr<osgWidget::WindowManager> vm =
            new osgWidget::WindowManager(&viewer, 1024.0f, 768.0f, 0xf0000000);

    osg::Camera* camera = vm->createParentOrthoCamera();
    vm->addChild(createSimpleTabs(100.0f, 100.0f));
    vm->resizeAllWindows();
    ...
    viewer.addEventHandler(new osgWidget::MouseHandler(vm.get()));
    viewer.addEventHandler(new osgWidget::ResizeHandler(vm.get(), camera));
    viewer.addEventHandler(new osgWidget::CameraSwitchHandler(vm.get(), camera));
